...
extension OWHSyncEngine {
    ...
    internal func serialize(samples: [HKSample], type: HKSampleType) -> [String: Any] {
        ...
        for s in samples {
            if let w = s as? HKWorkout {
                records.append(mapWorkout(w))
            } else if let q = s as? HKQuantitySample {
                records.append(mapQuantity(q))
            } else if let c = s as? HKCategorySample {
                records.append(mapCategory(c))
            } else if let corr = s as? HKCorrelation {
                records.append(contentsOf: mapCorrelation(corr))
            } else {
                records.append([
                    ...
                    "source": mapSource(s.sourceRevision, device: s.device),
                    "recordMetadata": metadataList(s.metadata)
                ])
            }
        }
    ...
    internal func serializeCombinedStreaming(samples: [HKSample]) -> [String: Any] {
        ...
        for batchStart in stride(from: 0, to: samples.count, by: batchSize) {
            autoreleasepool {
                ...
                for s in batch {
                    if let w = s as? HKWorkout {
                        workouts.append(mapWorkoutEfficient(w, dateFormatter: dateFormatter))
                    } else if let q = s as? HKQuantitySample {
                        records.append(mapQuantityEfficient(q, dateFormatter: dateFormatter))
                    } else if let c = s as? HKCategorySample {
                        if c.categoryType.identifier == HKCategoryTypeIdentifier.sleepAnalysis.rawValue {
                            sleep.append(mapCategoryEfficient(c, dateFormatter: dateFormatter))
                        } else {
                            records.append(mapCategoryEfficient(c, dateFormatter: dateFormatter))
                        }
                    } else if let corr = s as? HKCorrelation {
                        records.append(contentsOf: mapCorrelationEfficient(corr, dateFormatter: dateFormatter))
                    } else {
                        records.append([
                            "uuid": s.uuid.uuidString,
                            "type": s.sampleType.identifier,
    ...
    internal func serializeCombined(samples: [HKSample], anchors: [String: HKQueryAnchor]) -> [String: Any] {
        ...
        for s in samples {
            if let w = s as? HKWorkout {
                workouts.append(mapWorkout(w))
            } else if let q = s as? HKQuantitySample {
                records.append(mapQuantity(q))
            } else if let c = s as? HKCategorySample {
                if c.categoryType.identifier == HKCategoryTypeIdentifier.sleepAnalysis.rawValue {
                    sleep.append(mapCategory(c))
                } else {
                    records.append(mapCategory(c))
                }
            } else if let corr = s as? HKCorrelation {
    ...
    // MARK: - Type mapping
    public func mapTypes(_ names: [String]) -> [HKSampleType] {
    ...
    private func mapQuantity(_ q: HKQuantitySample) -> [String: Any] {
        let df = ISO8601DateFormatter()
        let (unit, unitOut) = defaultUnit(for: q.quantityType)
        ...
        if q.quantity.is(compatibleWith: unit) {
            ...
            let fallbackUnit = getFallbackUnit(for: q.quantityType)
        ...
        return [
            ...
            "source": mapSource(q.sourceRevision, device: q.device),
            "recordMetadata": metadataList(q.metadata)
        ]
    }

    private func mapCategory(_ c: HKCategorySample) -> [String: Any] {
        let df = ISO8601DateFormatter()
        return [
            ...
            "source": mapSource(c.sourceRevision, device: c.device),
            "recordMetadata": metadataList(c.metadata)
        ]
    }

    private func mapCorrelation(_ corr: HKCorrelation) -> [[String: Any]] {
        var records: [[String: Any]] = []
        let df = ISO8601DateFormatter()
        let source = mapSource(corr.sourceRevision, device: corr.device)
    ...
    private func mapWorkout(_ w: HKWorkout) -> [String: Any] {
        let df = ISO8601DateFormatter()
        let stats = buildWorkoutStats(w)

        return [
            "uuid": w.uuid.uuidString,
            "type": workoutTypeString(w.workoutActivityType),
            "startDate": df.string(from: w.startDate),
            "endDate": df.string(from: w.endDate),
            "source": mapSource(w.sourceRevision, device: w.device),
    ...
    private func getFallbackUnit(for qt: HKQuantityType) -> HKUnit {
        switch qt {
    ...
    private func defaultUnit(for qt: HKQuantityType) -> (HKUnit, String) {
        switch qt {
        case HKObjectType.quantityType(forIdentifier: .stepCount):
            return (.count(), "count")
        case HKObjectType.quantityType(forIdentifier: .heartRate):
    ...
    private func workoutTypeString(_ t: HKWorkoutActivityType) -> String {
        switch t {
    ...
    private func metadataList(_ meta: [String: Any]?) -> [[String: Any]] {
        var list: [[String: Any]] = []
    ...
    private func mapQuantityEfficient(_ q: HKQuantitySample, dateFormatter: ISO8601DateFormatter) -> [String: Any] {
        let (unit, unitOut) = defaultUnit(for: q.quantityType)
    ...
    private func mapCategoryEfficient(_ c: HKCategorySample, dateFormatter: ISO8601DateFormatter) -> [String: Any] {
        return [
            "uuid": c.uuid.uuidString,
            "type": c.categoryType.identifier,
            "value": c.value,
    ...
    private func mapCorrelationEfficient(_ corr: HKCorrelation, dateFormatter: ISO8601DateFormatter) -> [[String: Any]] {
        var records: [[String: Any]] = []
        let source = mapSource(corr.sourceRevision, device: corr.device)
    ...
    private func mapWorkoutEfficient(_ w: HKWorkout, dateFormatter: ISO8601DateFormatter) -> [String: Any] {
        let stats = buildWorkoutStats(w)
    ...
    // MARK: - Workout statistics builder (shared between standard and efficient mappers)

    private func buildWorkoutStats(_ w: HKWorkout) -> [[String: Any]] {
            ...
            if let hrType = HKQuantityType.quantityType(forIdentifier: .heartRate),
               let hrStats = w.statistics(for: hrType) {
                ...
                if let min = hrStats.minimumQuantity() {
                    stats.append(["type": "minHeartRate", "value": min.doubleValue(for: bpmUnit), "unit": "bpm"])
                }
                if let avg = hrStats.averageQuantity() {
                    stats.append(["type": "averageHeartRate", "value": avg.doubleValue(for: bpmUnit), "unit": "bpm"])
                }
                if let max = hrStats.maximumQuantity() {
                    stats.append(["type": "maxHeartRate", "value": max.doubleValue(for: bpmUnit), "unit": "bpm"])
                }
            }
        ...
        if let elevationAscended = w.metadata?[HKMetadataKeyElevationAscended] as? HKQuantity {
            stats.append(["type": "elevationAscended", "value": elevationAscended.doubleValue(for: .meter()), "unit": "m"])
        }
        if let elevationDescended = w.metadata?[HKMetadataKeyElevationDescended] as? HKQuantity {
            stats.append(["type": "elevationDescended", "value": elevationDescended.doubleValue(for: .meter()), "unit": "m"])
        }
        if let avgSpeed = w.metadata?[HKMetadataKeyAverageSpeed] as? HKQuantity {
            stats.append(["type": "averageSpeed", "value": avgSpeed.doubleValue(for: HKUnit.meter().unitDivided(by: .second())), "unit": "m/s"])
        }
        if let maxSpeed = w.metadata?[HKMetadataKeyMaximumSpeed] as? HKQuantity {
            stats.append(["type": "maxSpeed", "value": maxSpeed.doubleValue(for: HKUnit.meter().unitDivided(by: .second())), "unit": "m/s"])
        }
        if let avgMETs = w.metadata?[HKMetadataKeyAverageMETs] as? HKQuantity {
            stats.append(["type": "averageMETs", "value": avgMETs.doubleValue(for: HKUnit.kilocalorie().unitDivided(by: HKUnit.gramUnit(with: .kilo).unitMultiplied(by: .hour()))), "unit": "kcal/kg/hr"])
        }
        if let lapLength = w.metadata?[HKMetadataKeyLapLength] as? HKQuantity {
            stats.append(["type": "lapLength", "value": lapLength.doubleValue(for: .meter()), "unit": "m"])
        }
        if let swimmingLocationType = w.metadata?[HKMetadataKeySwimmingLocationType] as? NSNumber {
            stats.append(["type": "swimmingLocationType", "value": swimmingLocationType.intValue, "unit": "enum"])
        }
        if let indoorWorkout = w.metadata?[HKMetadataKeyIndoorWorkout] as? Bool {
            stats.append(["type": "indoorWorkout", "value": indoorWorkout ? 1 : 0, "unit": "bool"])
        }
        if let weatherTemp = w.metadata?[HKMetadataKeyWeatherTemperature] as? HKQuantity {
            stats.append(["type": "weatherTemperature", "value": weatherTemp.doubleValue(for: .degreeCelsius()), "unit": "degC"])
        }
        if let weatherHumidity = w.metadata?[HKMetadataKeyWeatherHumidity] as? HKQuantity {
            stats.append(["type": "weatherHumidity", "value": weatherHumidity.doubleValue(for: .percent()), "unit": "%"])
        }
    ...
    internal func mapSource(_ sourceRevision: HKSourceRevision, device: HKDevice?) -> [String: Any] {
